...
contract RebalancingPools {
  ...
  address[] availableTokens = [0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889, 0x001B3B4d0F3714Ca98ba10F6042DaEbF0B1B7b6F]; // Temporarly hardcoded: WMATIC, DAI - Mumbai
  ...
  ]; // Temporarly hardcoded: WMATIC/USD, DAI/USD - Mumbai
  ...
  function decodeLowAndHighDelta(address _user, uint256 _idx) external view returns (uint256[2] memory) {
    bytes memory byt;
    uint256 lowDelta = 0;
    uint256 highDelta = 0;
    (, bytes[] memory tooLowBalance, bytes[] memory tooHighBalance) = checkPercentageProportions(_user);
    if (keccak256(abi.encodePacked(byt)) != keccak256(abi.encodePacked(tooLowBalance[_idx]))) {
      (lowDelta, ) = abi.decode(tooLowBalance[_idx], (uint256, address));
    }
    if (keccak256(abi.encodePacked(byt)) != keccak256(abi.encodePacked(tooHighBalance[_idx]))) {
      (highDelta, ) = abi.decode(tooHighBalance[_idx], (uint256, address));
    }
  ...
  function decodeLowAndHighAddress(address _user, uint256 _idx) external view returns (address[2] memory) {
    bytes memory byt;
    address lowAddress;
    address highAddress;
    (, bytes[] memory tooLowBalance, bytes[] memory tooHighBalance) = checkPercentageProportions(_user);
    if (keccak256(abi.encodePacked(byt)) != keccak256(abi.encodePacked(tooLowBalance[_idx]))) {
      (, lowAddress) = abi.decode(tooLowBalance[_idx], (uint256, address));
    }
    if (keccak256(abi.encodePacked(byt)) != keccak256(abi.encodePacked(tooHighBalance[_idx]))) {
      (, highAddress) = abi.decode(tooHighBalance[_idx], (uint256, address));
    }
  ...
  function rebalance(address _user) external /* onlyKeeper */ {
    bytes memory byt;
    (, bytes[] memory tooLowBalance, bytes[] memory tooHighBalance) = checkPercentageProportions(_user);
    uint256 lowIdx = 0;
    uint256 highIdx = 0;
    ...
    for (; lowIdx < tooLowBalance.length && highIdx < tooHighBalance.length; ) {
      //   console.log("[Inside]Too low balance:");
      if (keccak256(abi.encodePacked(byt)) != keccak256(abi.encodePacked(tooLowBalance[lowIdx]))) {
        (lowDelta, lowTokenAddress) = abi.decode(tooLowBalance[lowIdx], (uint256, address));
      } else {
        lowIdx++;
      }
      if (keccak256(abi.encodePacked(byt)) != keccak256(abi.encodePacked(tooHighBalance[highIdx]))) {
        (highDelta, highTokenAddress) = abi.decode(tooHighBalance[highIdx], (uint256, address));
      } else {
        highIdx++;
      }
  ...
  function checkPercentageProportions(address _user) public view returns (bool, bytes[] memory, bytes[] memory) {
    ...
    for (; idx < l_pool.chosenTokens.length; idx++) {
      ...
      //   console.log("[Inside]Percentage: %s", l_pool.proportionsInPercentage[idx]);
