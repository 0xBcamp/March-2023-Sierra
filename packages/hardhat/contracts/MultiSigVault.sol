...
contract MultiSigVault is Ownable {
  ...
  address private Uniswap_V2_Router02 = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
  address private DAI = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
  address private ACCEPTABLE_TOKEN = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889; //DAI by default

  address private iwethAddress = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;
  ...
  function getBalance(address _tokenAddress) public view returns (uint256 balance) {
    return IERC20(_tokenAddress).balanceOf(address(this));
  }

  /// @notice -------------allows withdraw when approvalLimit is met----------
  function withdraw(address _tokenAddress) public returns (uint256) {
    /* Will not work in such state - swap and funds update are needed */
  ...
  function configureRebalancingPool(
    ...
    for (uint idx = 0; idx < _chosenTokens.length; idx++) {
      if (_chosenTokens[idx] != ACCEPTABLE_TOKEN) {
        proportions[idx] = (_proportionsInPercentage[idx] * _totalValue) / 1000;
        proportions[idx] = swap(ACCEPTABLE_TOKEN, _chosenTokens[idx], proportions[idx], 0 /* temporary */);
      } else {
